...
```text
Input:
ตัวเลข 3 ตัว แทนค่า R, G, B ของสีที่ 1
ตัวเลข 3 ตัว แทนค่า R, G, B ของสีที่ 2
Output:
ตัวเลขผลลัพธ์การผสมสี R, G, B จำนวน 3 ตัว
Constraints:
แต่ละสีจะถูกแทนด้วยตัวเลขจำนวนเต็มตั้งแต่ 0 (ไม่มีสีนั้นเลย หรือมืดสนิท) ไปจนถึง 255 (สว่างสว่างที่สุด)
```
...
```text
Step 1: อ่านค่าแม่สี R, G, B ของสีที่ 1 จากอินพุตบรรทัดแรก โดยแยกข้อมูลด้วยช่องว่าง
Step 2: อ่านค่าแม่สี R, G, B ของสีที่ 2 จากอินพุตบรรทัดที่สอง ในลักษณะเดียวกัน
Step 3: คำนวณหาค่าเฉลี่ย R โดยนำ R1 + R2 แล้วหารด้วย 2 (ปัดเศษลงให้เป็นจำนวนเต็ม)
Step 4: คำนวณหาค่าเฉลี่ย G โดยนำ G1 + G2 แล้วหารด้วย 2 (ปัดเศษลงให้เป็นจำนวนเต็ม)
Step 5: คำนวณหาค่าเฉลี่ย B โดยนำ B1 + B2 แล้วหารด้วย 2 (ปัดเศษลงให้เป็นจำนวนเต็ม)
Step 6: พิมพ์ค่าเฉลี่ย R, G, B ออกมาในบรรทัดเดียวกันโดยเว้นวรรคคั่นกลาง
```
...
```text
วิธีสุดท้ายของฉันเหมือนกับแผนแรก ฉันใช้ input().split() ร่วมกับ map(int, ...) เพื่ออ่านค่า R, G, B ของทั้งสองสีให้เป็นจำนวนเต็ม จากนั้นนำค่าสีแต่ละช่องมารวมกันแล้วหารด้วย 2 โดยใช้เครื่องหมาย // เพื่อปัดเศษลงให้ได้ผลลัพธ์เป็นจำนวนเต็มตามที่โจทย์ต้องการ ก่อนจะพิมพ์ค่าเฉลี่ย R, G, B ทั้งสามค่าออกทางหน้าจอคั่นด้วยช่องว่าง
```
...
```text
ทดสอบการทำงานปกติของการคำนวณค่าเฉลี่ย
```
...
```text
ทดสอบกรณีที่ผลรวมของค่าสีสองสีเป็นเลขคี่ เพื่อตรวจสอบว่าเครื่องหมาย // ปัดเศษลงเป็นจำนวนเต็มถูกต้องตามเงื่อนไขหรือไม่
```
