...
```text
Input:
โปรแกรมรับจำนวนเต็ม 2 ค่าในบรรทัดเดียวกัน คั่นด้วยช่องว่าง คือ $L$ (ขนาดหรือพารามิเตอร์ของแถบ) และ $N$ (เป้าหมายจำนวนช่องปลูกรวมทั้งหมด)
Output:
โปรแกรมควรพิมพ์จำนวนเต็ม 1 ค่า คือ ลำดับของแถบสุดท้าย ($k$) ที่ทำให้จำนวนช่องปลูกรวมสะสมมีค่ามากกว่าหรือเท่ากับ N
Constraints:
อินพุต L และ N เป็นจำนวนเต็มบวก (1 <= L <= 10; 1 <= N <= 100,000)
```
...
```text
Step 1: อ่านค่า L และ N จากอินพุตบรรทัดเดียวกัน โดยแปลงเป็นจำนวนเต็ม
Step 2: กำหนดค่าเริ่มต้นให้ลำดับแถบ k = 1 และผลรวมช่องปลูกสะสม total_planted = 0
Step 3: คำนวณจำนวนช่องในแถบที่ k ด้วยสูตร k * L^2 - L(L - 1)//2
Step 4: นำจำนวนช่องที่ได้ในแถบนั้นไปบวกเพิ่มใน total_planted
Step 5: ตรวจสอบว่า total_planted N หรือยัง
Step 6: ถ้าบวกสะสมแล้วได้มากกว่าหรือเท่ากับ N ให้พิมพ์ค่า k ออกมา แล้วหยุดการทำงาน
Step 7: ถ้ายังน้อยกว่า N ให้บวกเพิ่มค่า k ไปอีก 1 แล้ววนกลับไปคำนวณใน Step 3 ใหม่
```
...
```text
วิธีสุดท้ายของฉันเหมือนกับแผนแรก ใช้วิธีวนลูปคำนวณและบวกสะสมค่าทีละรอบฉันใช้ลูป while True เพื่อคำนวณจำนวนช่องปลูกในแถบที่ k จากสูตร k * L * L - (L * (L - 1)) // 2 แล้วนำไปบวกสะสมไว้ใน total_planted จากนั้นตรวจสอบว่าผลรวมเกินหรือเท่ากับ N หรือยัง หากถึงเป้าหมายแล้วจะพิมพ์ค่า k และหยุดการทำงานทันที แต่ถ้ายังไม่ถึงก็จะเพิ่มค่า k ขึ้นทีละ 1 แล้ววนกลับไปคำนวณแถบถัดไป
```
...
```text
ทดสอบกรณีค่าต่ำสุดเท่าที่เป็นไปได้ของโปรแกรม เพื่อดูว่าโปรแกรมสามารถคำนวณในรอบแรก
```
...
```text
ทดสอบการทำงานทั่วไปที่มีการวนลูปสะสมมากกว่า 1 รอบ และค่า N พอดีกับผลรวมสะสมของแถบที่ 2 เพื่อดูว่าโปรแกรมคำนวณสะสมในแต่ละรอบถูกต้อง
```
...
```text
ทดสอบกรณีที่จำนวนช่องเป้าหมาย $N$ เกินจำนวนช่องในแถบแรกมาเพียงเล็กน้อย เพื่อตรวจสอบเงื่อนไขการเปรียบเทียบ >= ว่าโปรแกรมไม่หยุดการทำงานเร็วเกินไป และไม่วนลูปเกินรอบโดยไม่จำเป็น
```
