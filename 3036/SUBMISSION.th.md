...
```text
input:
เลขที่อยู่ชั้นไดชั้นหนึ่งของประสาท
output:
กำแพงที่ถูกทำลายน้อยที่สุด
constraints:
ทางที่ต้องทำลายกำแพงสั้นที่สุดไปยังห้องที่ 1
```
...
```text
Step 1:รับค่าเลขห้องจุดเริ่มต้น
Step 2:นำเลขห้องไปหาชั้นที่อยู่
Step 3:นับขึ้นไปหาห้องที่1
```
...
```text
Step 1:รับค่าเลขห้องจุดเริ่มต้น
Step 2:หาชั้นที่เลขนั้นอยู่
Step 3:เมื่อหาเจอแล้วให้หาว่าเลขที่ห้องในชั้นนั้นเมื่อนับจากซ้ายมาขวาเป็นตำแหน่งเลขคู่หรือคี่
Step 4:นำเลขชั้นไปคูณ2 เพื่อหากำแพงที่ต้องทำลายไม่เกินนี้
Step 5:เมื่อตำแหน่งเลขคี่ให้ลบค่าในStep 4 ไป2 เมื่อเป็นเลขคู่ให้ลบไป3 เพราะมีกำแพงที่จุดเริ่มต้นทำลายแค่1ก็ทลุไปชั้นต่อไปแล้ว
```
...
```text
โจทย์ถ้าไม่ต้องทำลายกำแพงเลยให้ตอบ0
```
...
```text
เช็คเงื่อนไขเมื่อเลขอยู่ตำแหน่งคี่
```
...
```text
เช็คเงื่อนไขเมื่อเลขอยู่ตำแหน่งคู่
```
...
```text
อธิบายว่าจะหาชั้นได้ยังไง
```

คุณยังทำอะไรด้วยตนเอง

```text
หาวิธีหาตำแหน่งคู่คี่ เขียนโค้ดด้วยตัวเอง testด้วยตนเอง นำส่งOJด้วยตัวเอง
```
